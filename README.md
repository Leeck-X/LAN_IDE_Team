...
- **文件类型适配（云盘式预览/下载）**：png / jpg / gif / webp / svg 图片在线预览，视频 / 音频 / PDF 内置播放器或内嵌预览；exe / zip / dll 等二进制类型给出「暂不支持在线预览」提示；预览面板工具栏显示文件名、MIME 类型与大小，提供「下载」「新窗口打开」按钮；文件树 hover 任意文件出现 ⬇ 按钮即可直接下载，无需打开
- **用户身份与改名**：左下角点击自己的名字条即可改名，立即对全体协作者生效（名字持久化到 localStorage）
- **设备识别**：鼠标悬停他人名字条显示设备名称（服务器通过 NetBIOS/DNS 反查局域网 IP 主机名，查不到时回退浏览器 UA 标签）与局域网 IP 尾（如 `.42`）
- **管理员与只读模式**：在部署服务器的本机访问网页即为管理员（👑 标记）；管理员点击其他用户弹出用户卡片（IP、设备、使用统计），可一键将该用户**设为只读**（🔒 标记）或解除；只读用户的服务端全接口（编辑 / 保存 / 新建 / 重命名 / 删除 / 上传 / 测试点）均被拦截返回 403，客户端编辑器同步切为只读并实时收到通知
- **用户数据统计**：以 IP 为键本地存储到 `users.json`（每 5 秒自动落盘）——只读标记、最近名字、代码量（协同编辑插入字符数）、新建 / 删除 / 上传 / 保存次数、最后活跃时间，管理员可在用户卡片中查看
- **转到定义**：clangd LSP 支持转到定义（`F12`），编辑器内直接跳转到函数 / 变量定义处
- **模板库管理**：「模板」按钮支持按扩展名查看 / 新增 / 修改 / 删除文件模板，配置持久化到服务端
...
- **二进制文件预览/下载**：`/api/file` 对二进制文件返回 MIME 与大小，`/api/file/raw` 支持 `?download=1` 附件下发（云盘式下载）；前端按类型渲染预览面板（图片/视频/音频/PDF），不支持类型给出提示
- **用户身份体系**：`/api/me` 返回是否管理员（服务器本机 IP）/ 是否被设只读；socket `join` 上报设备标签，服务端反查局域网主机名
- **管理员接口**：`/api/admin/users` 查看全部用户数据与在线连接；`/api/admin/readonly` 设/解只读（仅服务器本机可调用，禁止对自身设置），变更实时推送 `readonly_changed` 并刷新房间 presence
- **只读强制执行**：11 个 HTTP 写接口（保存/备份/新建/重命名/删除/上传/上传测试包/测试点 save·delete·add·import）+ socket `edit`/`save` 双层拦截，防止绕过前端直接调 API
- **使用统计**：`on_edit` 按插入字符数累计代码量，新建/删除/上传/保存各计数，`users.json` 每 5 秒落盘（关键操作立即落盘）
- **改名**：socket `rename` 事件更新所有所在房间并广播 presence，避免重进页面
- **presence 增强**：事件负载附带每个在线者的 `sid/ip_tail/device/readonly/admin`，前端名字条悬停显示设备与 IP 尾，管理员可点击弹出用户卡片
- **转到定义**：socket `lsp_definition` + clangd `textDocument/definition`
- **模板库 CRUD**：`/api/templates`（GET 列表 / PUT 保存 / DELETE 删除），前端模板面板可视化管理
